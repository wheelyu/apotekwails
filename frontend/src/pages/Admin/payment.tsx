...
import { GetAllInvoiceToday } from "../../../wailsjs/go/backend/VisitService";
...
const PaymentPage = () => {
    ...
    const getVisit = async () => {
        try {
            ...
            const response = await GetAllInvoiceToday(dateStr);
    ...
    const tabs = [
        ...
        { id: 'Belum Bayar', label: 'Belum Bayar', count: dataVisit.filter(q => q.is_paid === false).length, color: 'bg-yellow-500' },
        { id: 'Sudah Bayar', label: 'Sudah Bayar', count: dataVisit.filter(q => q.is_paid === true).length, color: 'bg-green-500' },
    ];
    ...
    const filteredData = dataVisit.filter(item => {
        ...
        const matchIsPaid = activeTab === 'all' ? item.status !== 'Dibatalkan' : activeTab === 'Belum Bayar' ? item.is_paid === false : item.is_paid === true;

        return matchSearch && matchDepartment && matchIsPaid;
    });
            ...
            <div className="p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    ...
                    <div className="bg-gradient-to-br from-yellow-500 to-yellow-600 rounded-xl p-5 text-white shadow-lg">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-yellow-100 text-sm mb-1">Belum Bayar</p>
                                <h3 className="text-3xl font-bold">{dataVisit.filter(q => q.is_paid === false).length}</h3>
                            </div>
                            <div className="bg-white/20 p-3 rounded-lg">
                    ...
                    <div className="bg-gradient-to-br from-green-500 to-green-600 rounded-xl p-5 text-white shadow-lg">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-green-100 text-sm mb-1">Sudah Bayar</p>
                                <h3 className="text-3xl font-bold">{dataVisit.filter(q => q.is_paid === true).length}</h3>
                            </div>
                            <div className="bg-white/20 p-3 rounded-lg">
